function [g,Npc,pc_id,p,z,g_id,g_nid] = tl_gibbs_energy(T,P,phase,td,p,g0,v0,rho_w,eps_di,spcs,Nspcs,g0_spcs)
...
%
%   Written by J. C. Vrijmoed (2022), update 2024
if ~exist('td','var')    
...
for i_sol = 1:length(phase)
    ...
    if mod_id == 8
        Nfl_in         = p{i_sol}*td(i_sol).n_em;
        indep_id       = td(i_sol).icomp_indep;
        B              = Nfl_in(:,indep_id);
        solvent        = td(i_sol).p_name(end);
        [gfl,Nfl,m_all,chk,logK,v] = tl_fluid_spec(T,B',solvent,spcs,indep_id,g0_spcs,Nspcs,rho_w,eps_di);
        g{i_sol} = gfl';
        Npc{i_sol} = Nfl';
    else
