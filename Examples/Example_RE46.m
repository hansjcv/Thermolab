...
phs_name    = {'Olivine','Orthopyroxene','Clinopyroxene','Feldspar','Ilmenite','Melt(G25)'};
...
options.x0 = 0.1;
options.x0_old = 1;
VolumefractionsAll = zeros(length(T),length(phs_name));
%% Minimization refinement
for iT = 1:length(T)
[alph_all,Npc_all,pc_id_ref,p_ref,g_min,alph_fsolve,Npc_fsolve,pc_id_fsolve,p_fsolve] = tl_minimizer(T(iT),P,Nsys,phs_name,p,td,options);
%% Postprocess
...
if ~isempty(alph_fsolve) && sum(alph_fsolve<0)==0 && isreal(alph_fsolve)
    ...
    [pc_id,phi,Cwt,sfu,rho,mu,p_out,phiw,g,molphase] = postprocess_fun(T(iT),P,td_stable,alph_fsolve,Npc_fsolve,molm,p_fsolve,1:numel(pc_id_fsolve),Stablephases,solv_tol,'PS94','S14');
    postprocess_reactions(T(iT),P,td_stable,pc_id,p_out,print_reactions);
else    
    [pc_id,phi,Cwt,sfu,rho,mu,p_out,phiw,g,molphase] = postprocess_fun(T(iT),P,td,alph_all,Npc_all,molm,p_ref,pc_id_ref,phs_name,solv_tol,'PS94','S14');
    postprocess_reactions(T(iT),P,td,pc_id,p_out,print_reactions);
    Stablephases    = phs_name(pc_id)';
end
% Here some more human readable meaning of the output from the postprocess function
for ip = 1:length(Stablephases)
    VolumefractionsAll(iT,strcmp(phs_name,Stablephases(ip))) = phi(ip);
end
Volumefractions = phi; 
...
area(T-273.15,VolumefractionsAll),legend(phs_name),axis tight